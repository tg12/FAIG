...
class Prediction(object):
    ...
    def __init__(self, config):
        ...
        self.hightrend_watermark = float(
            config['Trade']['hightrend_watermark'])
    ...
    def get_tradedata(self):
        return {"direction": self.direction_to_trade,
                "epic": self.epic_id,
    ...
    def trade_type_by_sentiment(self):
        if self.shortPositionPercentage > self.longPositionPercentage and self.shortPositionPercentage >= self.clientsentiment_value:
            ...
            print ("!!DEBUG shortPositionPercentage:{} longPositionPercentage:{} clientsentiment_value:{} hightrend_watermark:{}".format(
                self.shortPositionPercentage, self.longPositionPercentage, self.clientsentiment_value, self.hightrend_watermark))
            return None
    ...
    def determine_trade_direction(self):
        ...
        print ("price_diff:{} score:{} current_price:{} limitDistance:{} predict_accuracy:{} price_prediction:{}".format(
            price_diff, score, current_price, self.limitDistance, self.predict_accuracy, price_prediction))

        self.limitDistance = round(
            price_diff * score * self.greed,
    ...
    def linear_regression(self, x, y, high_price, low_price):
        ...
        print ("PRICE PREDICTION FOR PRICE " + self.epic_id +
               " IS : " + str(self.price_prediction))

        self.score = genius_regression_model.score(x, y)
        predictions = {'intercept': genius_regression_model.intercept_,
