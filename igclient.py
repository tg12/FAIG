...
def trackcall(f):
    ...
    def wrap(*args, **kwargs):
        while len(args[0].recent_calls) >= 30 - 1:
            time.sleep(1)
            args[0].recent_calls = [
                x for x in args[0].recent_calls if x > int(
                    time.time() - 60)]
...
class IGClient(object):
    ...
    def session(self, set_default=True):
        ...
        if set_default:
            ...
            self.update_session(
                {"accountId": self.accountId, "defaultAccount": "True"})
    ...
    def handleDealingRules(self, data):
        ...
        if dealingRules[r]['unit'] == 'PERCENTAGE':
            if current_price / 100 * \
                    float(dealingRules[r]['value']) < float(data['limitDistance']):
                data['limitDistance'] = str(
                    format(
                        current_price /
                        100 *
                        float(
                            dealingRules[r]['value']),
                        '.2f'))
        ...
        if dealingRules[r]['unit'] == 'PERCENTAGE':
            if current_price / 100 * \
                    float(dealingRules[r]['value']) > float(data['stopDistance']):
