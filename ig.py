...
class API(IGClient):
    ...
    def placeOrder(self, prediction):
        ...
        print(
            "DEAL ID : {} - {} - {}".format(str(d['dealId']), d['dealStatus'], d['reason']))

        if str(
            d['reason']) == "ATTACHED_ORDER_LEVEL_ERROR" or str(
            d['reason']) == "MINIMUM_ORDER_SIZE_ERROR" or str(
            d['reason']) == "INSUFFICIENT_FUNDS" or str(
                d['reason']) == "MARKET_OFFLINE":
            print (
    ...
    def find_next_trade(self):
        ...
        while(1):
            ...
            for epic_id in epic_ids:
                ...
                if (Price_Change_Day_percent_h > Price_Change_Day_percent > Price_Change_Day_percent_l) or (
                        (Price_Change_Day_percent_h * -1) < Price_Change_Day_percent < (Price_Change_Day_percent_l * -1)):
                    ...
                    if eval(self.config['Trade']['use_max_spread']):
                        ...
                        max_permitted_spread = float(
                            epics[epic_id]['minspread'] * float(self.config['Trade']['spread_multiplier']) * -1)
